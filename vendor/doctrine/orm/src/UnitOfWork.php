...
class UnitOfWork implements PropertyChangedListener
{
    ...
    public function commit(): void
    {
        ...
        if ($this->evm->hasListeners(Event::preFlush)) {
            $this->evm->dispatchEvent(Event::preFlush, new PreFlushEventArgs($this->em));
        }
    ...
    public function computeChangeSet(ClassMetadata $class, object $entity): void
    {
        ...
        $invoke = $this->listenersInvoker->getSubscribedSystems($class, Event::preFlush) & ~ListenersInvoker::INVOKE_MANAGER;

        if ($invoke !== ListenersInvoker::INVOKE_NONE) {
            $this->listenersInvoker->invoke($class, Event::preFlush, $entity, new PreFlushEventArgs($this->em), $invoke);
        }
    ...
    {
        $oid    = spl_object_id($entity);
        $invoke = $this->listenersInvoker->getSubscribedSystems($class, Event::prePersist);

        if ($invoke !== ListenersInvoker::INVOKE_NONE) {
            $this->listenersInvoker->invoke($class, Event::prePersist, $entity, new PrePersistEventArgs($entity, $this->em), $invoke);
        }
    ...
    private function executeInserts(): void
    {
        ...
        foreach ($entities as $entity) {
            ...
            $invoke = $this->listenersInvoker->getSubscribedSystems($class, Event::postPersist);

            if ($invoke !== ListenersInvoker::INVOKE_NONE) {
                $eventToDispatch[] = ['class' => $class, 'entity' => $entity, 'invoke' => $invoke];
            }
        }

        // Defer dispatching `postPersist` events to until all entities have been inserted and post-insert
        // IDs have been assigned.
        foreach ($eventToDispatch as $event) {
    ...
    private function executeUpdates(): void
    {
        foreach ($this->entityUpdates as $oid => $entity) {
            $class            = $this->em->getClassMetadata($entity::class);
            $persister        = $this->getEntityPersister($class->name);
            $preUpdateInvoke  = $this->listenersInvoker->getSubscribedSystems($class, Event::preUpdate);
            $postUpdateInvoke = $this->listenersInvoker->getSubscribedSystems($class, Event::postUpdate);

            if ($preUpdateInvoke !== ListenersInvoker::INVOKE_NONE) {
                $this->listenersInvoker->invoke($class, Event::preUpdate, $entity, new PreUpdateEventArgs($entity, $this->em, $this->getEntityChangeSet($entity)), $preUpdateInvoke);
            ...
            if ($postUpdateInvoke !== ListenersInvoker::INVOKE_NONE) {
                $this->listenersInvoker->invoke($class, Event::postUpdate, $entity, new PostUpdateEventArgs($entity, $this->em), $postUpdateInvoke);
            }
        }
    }
    ...
    private function executeDeletions(): void
    {
        ...
        foreach ($entities as $entity) {
            ...
            $class     = $this->em->getClassMetadata($entity::class);
            $persister = $this->getEntityPersister($class->name);
            $invoke    = $this->listenersInvoker->getSubscribedSystems($class, Event::postRemove);
    ...
    private function doRemove(object $entity, array &$visited): void
    {
        ...
        switch ($entityState) {
            ...
            case self::STATE_MANAGED:
                $invoke = $this->listenersInvoker->getSubscribedSystems($class, Event::preRemove);

                if ($invoke !== ListenersInvoker::INVOKE_NONE) {
                    $this->listenersInvoker->invoke($class, Event::preRemove, $entity, new PreRemoveEventArgs($entity, $this->em), $invoke);
                }
    ...
    public function clear(): void
    {
        ...
        if ($this->evm->hasListeners(Event::onClear)) {
            $this->evm->dispatchEvent(Event::onClear, new OnClearEventArgs($this->em));
        }
    }
    ...
    private function dispatchOnFlushEvent(): void
    {
        if ($this->evm->hasListeners(Event::onFlush)) {
            $this->evm->dispatchEvent(Event::onFlush, new OnFlushEventArgs($this->em));
        }
    }
    ...
    private function dispatchPostFlushEvent(): void
    {
        if ($this->evm->hasListeners(Event::postFlush)) {
            $this->evm->dispatchEvent(Event::postFlush, new PostFlushEventArgs($this->em));
        }
    }
